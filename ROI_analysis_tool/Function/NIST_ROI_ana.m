...
% figure;imagesc(map_data); colormap hot; axis equal tight;
% viscircles(NIST_std_par.template_loc, repmat(Sphere1_radii, NIST_std_par.num_spheres, 1));

%% Calculate transformation matrix for rotation
rot_center = NIST_std_par.temp_rot_center;
template_vec = [NIST_std_par.template_Sphere1_loc(1)-NIST_std_par.temp_rot_center(1),...
    NIST_std_par.template_Sphere1_loc(2)-NIST_std_par.temp_rot_center(2)]; % Vector for template 
...
map_loc = (NIST_std_par.template_loc - rot_center) * trans_matrix + rot_center; % rotation is respect to origin
map_radii = repmat(Sphere1_radii, NIST_std_par.num_spheres, 1);

%% Visualize the new locations
figure;imagesc(map_data); colormap hot; axis equal tight;
